...
def run_baseline(data_dir: str) -> dict:
    ...
    metrics = run_experiment_1_baseline(data_dir)
    logger.info(f"✅ Baseline  F1={metrics['f1']:.4f}  ROC-AUC={metrics['roc_auc']:.4f}")
    return metrics


def run_engineered(data_dir: str):
    logger.info("═" * 60)
    ...
    rf_model, metrics = run_experiment_2_engineered(data_dir)
    logger.info(f"✅ Engineered F1={metrics['f1']:.4f}  ROC-AUC={metrics['roc_auc']:.4f}")
    return rf_model, metrics


def run_ensemble(data_dir: str, rf_model) -> dict:
    logger.info("═" * 60)
    ...
    metrics = run_experiment_3_ensemble(data_dir, rf_model)
    logger.info(f"✅ Ensemble  F1={metrics['f1']:.4f}  ROC-AUC={metrics['roc_auc']:.4f}")
    return metrics


def save_feature_cols(data_dir: str):
...
def main():
    ...
    parser.add_argument("--data", default="data/raw", help="Path to CICIDS2017 CSV directory")
    ...
    try:
        ...
        if args.exp in ("all", "baseline"):
            results["baseline"] = run_baseline(data_dir)

        if args.exp in ("all", "engineered"):
            rf_model, metrics = run_engineered(data_dir)
        ...
        if args.exp in ("all", "ensemble"):
            ...
            results["ensemble"] = run_ensemble(data_dir, rf_model)

        # Always save feature columns after training
        save_feature_cols(data_dir)
